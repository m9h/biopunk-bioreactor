...
| [`docs/ai-crispr-evolution-project.md`](docs/ai-crispr-evolution-project.md) | A concrete project: AI-directed continuous evolution with a CRISPR-driven loop (EvolvR diversification, in-cell recorder, fleet + archive). **The goal.** |
...
| [`docs/build-plans.md`](docs/build-plans.md) | Milestoned, actionable build plans for the three champions — updated Chi.Bio (proto), the evolution machine + evidence apparatus (devo), updated Pioreactor (bior) — with success metrics and guardrails. **The roadmap.** |
