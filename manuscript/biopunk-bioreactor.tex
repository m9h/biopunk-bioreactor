...
one compromise machine, and Section~\ref{sec:builds} gives the three concrete builds it implies
and the benefit each delivers. Section~\ref{sec:education} documents the educational materials
these platforms already serve, and Section~\ref{sec:aidirected} sets out the goal the whole
programme points toward --- AI-directed continuous evolution, motivated by OpenEvo's own pitch. We are
explicit throughout about what was verified against a primary source and what was not.
...
\subsection{An intersection: {CRISPR} and continuous evolution}
\label{sec:crispr}

The \texttt{bior} landmark above is itself a CRISPR result, and that is no accident: continuous
culture and CRISPR are entangled in three directions. \emph{These tools evolve CRISPR.}
Phage-assisted continuous evolution, run in continuous culture, is now a primary engine for
improving Cas enzymes --- the broad-PAM, high-specificity xCas9 was evolved by PACE
\cite{hu2018xcas9}, cytosine base editors were evolved for expanded target compatibility and
activity \cite{thuronyi2019bepace}, and the compact Cas9 variants of the landmark itself came from
scaling PACE onto eVOLVER \cite{huang2022epace}. Many of the CRISPR reagents now in routine use
were, in part, made in bioreactors of this kind. \emph{CRISPR is used inside these tools.} Pooled
genetic libraries --- the substrate of CRISPR screens --- can be maintained and analysed in
continuous culture \cite{mcgeachy2019}, and Cas-based selection, linking editing or interference to
survival, is a natural way to impose the growth-coupled selection that an evolution machine needs.
\emph{And CRISPR may help close the apparatus's hardest gap.} The archiving problem of
Section~\ref{sec:apparatus} --- the need for a frozen fossil record --- has a molecular alternative
in CRISPR-based recorders that write lineage and event history into a cell's own genome; whether an
in-cell ledger can substitute for a physical archive on a cheap open platform is an open and
appealing question.
...
\section{Three concrete builds and their benefits}
\label{sec:builds}

The proposal above is a design argument; here are the three buildable instruments it implies ---
the biopunk variants of each champion --- stated with the benefit each delivers over its base
platform. The milestoned engineering detail is in the repository
(\code{docs/build-plans.md} and \code{docs/best-hybrids.md}); what follows is what each variant
\emph{buys} you.

\textbf{The \texttt{proto} variant --- an updated Chi.Bio.} Keep Chi.Bio's non-contact optical core
and MPC, graft OpenEvo's closed, gas-capable vessel, add dissolved-oxygen and pH optodes read
through the wall, add a dispersive C12880MA spectrometer alongside the AS7341, and port the compute
to a Raspberry Pi. \emph{Benefit:} it closes Chi.Bio's one real weakness --- the open-to-atmosphere
tube --- turning gas into a controlled variable, and it adds the two physiological readouts (DO,
pH) and the continuous spectrum that let a user follow a culture's metabolic state and resolve
overlapping reporters rather than infer them. The result is the most complete non-contact in-situ
characterisation instrument in open hardware: eight gas-controlled, richly-instrumented, closed
reactors on one computer, with no loss of the hot-swappable, contamination-resistant discipline
that made Chi.Bio worth building on.

\textbf{The \texttt{devo} variant --- the evolution machine with an evidence apparatus.} Build on a
Pioreactor cluster (or an OpenEvo base for teaching and shoestring use), graft OpenEvo's three-media
fluidics and closed vessel, and add the evidence apparatus: a replicate manifold of at least eight
lineages, an automated chilled archiver on the waste line, a \code{breseq} variant-calling pipeline,
and an automated revive-ancestor fitness assay. \emph{Benefit:} this is the variant that fills the
empty \texttt{devo} slot. Replicates convert an anecdote into causal evidence through convergence;
the archiver --- the field's missing freezer, and the one genuinely new module --- makes
ancestor-versus-descendant fitness measurable rather than merely asserted; \code{breseq} calls the
insertion-sequence structural variants a default mapper misses; and a batch-excursion mode recovers
carrying-capacity phenotypes a turbidostat structurally cannot see. Together they make it the first
open platform that produces \emph{publishable} evolution evidence --- and, as
Section~\ref{sec:aidirected} argues, the substrate on which AI-directed evolution can finally be
built.

\textbf{The \texttt{bior} variant --- Pioreactor plus add-ons.} Buy a Pioreactor cluster and, rather
than fabricate, add capability through its plugin path: the same optode DO/pH module the
\texttt{proto} variant uses (identical footprint, no new board), and eVOLVER Smart Sleeves on only
those vials that genuinely need arbitrary per-vial control. \emph{Benefit:} reliable parallel
cultivation available now, with minimal build effort and a maintained software path, optionally
lifted to physiological sensing or per-vial configurability exactly where a study needs it and
nowhere else. The value of the \texttt{bior} variant is precisely that most of it is bought, not
built.

The benefit that ties the three together is the shared ecosystem made concrete. Because all three
stand on the same Raspberry-Pi compute family, the same module footprint, and one calibration
reference, a laboratory that builds one variant has most of what the next requires: the optode
module designed for \texttt{proto} drops onto the \texttt{bior} fleet unchanged, and the calibration
reference makes every instrument's optical-density reading agree before any of them fork. The
variants are not three separate products but three configurations of one open platform --- which is
the whole payoff of refusing to build a single machine that tries to do everything.
...
\section{Toward AI-directed continuous evolution}
\label{sec:aidirected}

OpenEvo closes its discussion with an ambition it does not build toward: its automation, the
authors write, ``opens the door to implementing AI-directed evolution experiments, enabling AI to
supervise the evolution, increasing the speed, or pushing the limits on the qualities/functions
that can be evolved,'' with a UV-mutagenesis ``feedback system that detects when the culture fails
to evolve quickly and then induces mutations'' as the one concrete mechanism. We take this as the
goal --- and observe that the preprint pitches only the actuator half of it.

AI-directed evolution is a closed-loop optimisation problem: a controller must \emph{observe} the
state of an evolving population and \emph{act} on the pressures that shape it. OpenEvo can
act --- dilution, media, light --- but produces no learnable signal, because it has no replicates,
no archive, and no sequencing feedback; a controller cannot steer what it cannot see. This is the
same deficit that leaves the \texttt{devo} slot empty (Section~\ref{sec:devo}), which yields the
central point of this section: \textbf{the observation layer an AI controller requires is exactly
the evidentiary apparatus that makes a \texttt{devo} champion.} Championship and intelligence are
different axes --- the apparatus produces evidence, the controller consumes it to search --- and
the first is the prerequisite for the second. Building AI-directed evolution therefore necessarily
builds the champion as its substrate; a fixed-schedule version of the same apparatus is already
the champion, and the controller only makes it a better one. The correct build order is the inverse
of OpenEvo's pitch: build the apparatus first, prove it produces evidence, then add the autopilot.

The loop reuses what this paper has already specified. The controller's \emph{state} is the
apparatus's output --- real-time growth rate, conductivity, and dissolved oxygen and pH from the
shared optode modules, together with periodic whole-genome sequencing (via \code{breseq}) and
fitness coefficients from revived-ancestor competitions. Its \emph{actions} are the actuator's
knobs --- selection-media scheduling and blended gradients, optogenetic light dose, dilution
set-point, temperature, and mutation supply. Its \emph{policy} escalates as data
accumulate: rule-based control first, then Bayesian optimisation over the selection-regime space,
then reinforcement or active learning, all running on the leader node of the replicate fleet, which
supplies the parallel arms an explore--exploit search requires. This differs from established
machine-learning-guided directed evolution \cite{yang2019ml}, an offline design--build--test cycle
over protein sequences; here the learner sits inside a live, continuous culture.

CRISPR sharpens every layer, turning blunt knobs into precise ones. For \emph{mutation supply},
CRISPR-guided DNA polymerases such as EvolvR diversify a chosen locus at tunable rates up to
millions of fold over background \cite{halperin2018evolvr}, giving the controller a targeted
diversification dial in place of genome-wide UV. For \emph{observation}, CRISPR molecular recorders
that write stimulus history into a cell's own genome --- such as the base-editing CAMERA system
\cite{tang2018camera} --- offer a cheap, high-frequency in-cell readout of the population's
trajectory between sequencing runs. For \emph{selection}, coupling Cas activity to survival lets the
controller compose arbitrary, programmable pressures. A concrete project assembled from these pieces
is set out in the repository (\code{docs/ai-crispr-evolution-project.md}).

Two guardrails, both raised earlier, become control-theoretic constraints here. A controller that
raises mutation supply whenever adaptation stalls will hill-climb into mutational meltdown --- the
hypermutator failure mode of six of the twelve LTEE populations \cite{blount2012cit} --- unless it
optimises against a model of mutation load rather than a bare threshold. And because selection
rewards growth, product formation must be growth-coupled before the run, or the controller will
efficiently find the pathway-deletion mutant \cite{sandberg2019ale}. The demonstration that would
establish the program is a head-to-head: AI-directed selection against a fixed schedule, on a
defined target and across a replicate fleet, showing the controller reaches the phenotype faster
or reaches one the fixed schedule cannot.
...
\bibitem{hu2018xcas9} Hu JH, Miller SM, Geurts MH, et al., Liu DR. Evolved Cas9 variants with
  broad PAM compatibility and high DNA specificity. \textit{Nature} 556:57--63 (2018).
\bibitem{thuronyi2019bepace} Thuronyi BW, Koblan LW, Levy JM, et al., Liu DR. Continuous
  evolution of base editors with expanded target compatibility and improved activity.
  \textit{Nature Biotechnology} 37:1070--1079 (2019).
\bibitem{mcgeachy2019} McGeachy AM, Meacham ZA, Ingolia NT. An accessible continuous-culture
  turbidostat for pooled analysis of complex libraries. \textit{ACS Synthetic Biology}
  8(4):844--856 (2019).
\bibitem{yang2019ml} Yang KK, Wu Z, Arnold FH. Machine-learning-guided directed evolution for
  protein engineering. \textit{Nature Methods} 16:687--694 (2019).
\bibitem{halperin2018evolvr} Halperin SO, Tou CJ, Wong EB, Modavi C, Schaffer DV, Dueber JW.
  CRISPR-guided DNA polymerases enable diversification of all nucleotides in a tunable window.
  \textit{Nature} 560:248--252 (2018).
\bibitem{tang2018camera} Tang W, Liu DR. Rewritable multi-event analog recording in bacterial and
  mammalian cells. \textit{Science} 360:eaap8992 (2018).
\end{thebibliography}
